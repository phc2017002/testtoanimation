...
class MergeSortExplanation(VoiceoverScene):
    ...
    def create_array_visual(self, nums, pos=ORIGIN, scale_factor=0.3, color=BLUE, height_scale=1.0):
        ...
        for i, num in enumerate(nums):
            ...
            label = Text(str(num), font_size=24).move_to(rect.get_center())
    ...
    def algorithm_overview(self):
        ...
        return merge(left, right). The merge function interleaves two sorted halves into one sorted array.""" ) as tracker:
            self.play(Write(title))
            pseudocode = Text("mergesort(arr):\n  if len(arr) <= 1: return arr\n  mid = len(arr)//2\n  left = mergesort(arr[:mid])\n  right = mergesort(arr[mid:])\n  return merge(left, right)", font_size=24, line_spacing=1.2).next_to(title, DOWN, buff=0.5)
        ...
        No in-place swaps like BubbleSort; it uses extra space for temp arrays during merge.""" ) as tracker:
            base_case = MathTex(r"Base: n \\leq 1 \\to sorted").shift(LEFT * 2)
            recurse = MathTex(r"T(n) = 2T(n/2) + \\Theta(n)").shift(RIGHT * 2)
    ...
    def conquer_step(self):
        ...
        left_quarter = self.create_array_visual([3,8,4,7], LEFT * 2.5, height_scale=1.2)
        right_quarter = self.create_array_visual([1,6,2,5], RIGHT * 2.5, height_scale=1.2)
        ...
        Recursion ensures subproblems solved before combining.""" ) as tracker:
            left_quarter.set_color(GREEN)
            right_quarter.set_color(GREEN)
    ...
    def small_example(self):
        ...
        tree_nodes = VGroup(
            Text("[38,27,43,3]", font_size=24).move_to(UP * 1),
    ...
    def time_complexity(self):
        ...
        y_label = axes.get_y_axis_label(MathTex(r"T(n)"), direction=LEFT).shift(LEFT * 0.8).shift(LEFT * 0.8)
        
        nlogn = axes.plot(lambda x: x * np.log2(x + 1), color=BLUE, x_range=[1,10])
        ...
        MergeSort unbeatable worst-case.""" ) as tracker:
            ...
            legend = VGroup(
                Text("MergeSort: O(n log n)", font_size=24, color=BLUE).to_edge(LEFT),
                Text("BubbleSort: O(n²)", font_size=24, color=RED).next_to(Text("MergeSort: O(n log n)", font_size=24, color=BLUE), DOWN, buff=0.2)
            ).next_to(axes, RIGHT, buff=0.5).to_edge(UP, buff=2.0)
    ...
    def applications_and_summary(self):
        ...
        title = Text("Applications & Conclusion", font_size=36, color=GREEN).to_edge(UP, buff=1.0)
...
# Instructions to run:
# 1. Install Manim and manim-voiceover: pip install manim manim-voiceover
# 2. Save as mergesort.py
# 3. Render: manim -pql mergesort.py MergeSortExplanation
#    (Use -pql for preview low quality; -pqh for high quality)
# Note: Voiceover uses GTTS (internet required first run for cache). Total ~8-10 min duration.
